...
class ImprovedD2E2S_Trainer(BaseTrainer):
    def __init__(self, args: argparse.Namespace):
        super().__init__(args)
        self._tokenizer = AutoTokenizer.from_pretrained("microsoft/deberta-v3-large")
    ...
    def _preprocess(self, args, input_reader_cls, types_path, train_path, test_path):
        ...
        print("   Using improved GCN modules with configuration:")
        print(f"   - GCN Type: {getattr(args, 'gcn_type', 'improved')}")
        print(f"   - GCN Layers: {getattr(args, 'gcn_layers', 3)}")
        print(f"   - Attention Heads: {getattr(args, 'attention_heads', 8)}")
        print(f"   - Use Residual: {getattr(args, 'use_residual', True)}")
        print(f"   - Use Layer Norm: {getattr(args, 'use_layer_norm', True)}")
        print(f"   - Use Multi-scale: {getattr(args, 'use_multi_scale', True)}")
        print(f"   - Use Graph Attention: {getattr(args, 'use_graph_attention', True)}")
