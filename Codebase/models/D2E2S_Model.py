from transformers import DebertaV2Model, DebertaV2PreTrainedModel, AutoModel, AutoConfig
...
class D2E2SModel(DebertaV2PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, entity_masks=None, entity_sizes=None, 
        ...
        if hasattr(self, 'deberta_projection'):
            sequence_output = self.deberta_projection(sequence_output)

        if not evaluate:
            return self._forward_train(sequence_output, attention_mask, entity_masks, entity_sizes, 
                                     sentiments, senti_masks, adj)
        else:
            return self._forward_eval(sequence_output, attention_mask, entity_masks, entity_sizes, 
    ...
    def _forward_train(self, h, attention_mask, entity_masks, entity_sizes, sentiments, senti_masks, adj):
        ...
        h_sem_ori, adj_sem_ori = self.Sem_gcn(h, attention_mask, seq_lens)
        h_sem_gcn, adj_sem_gcn = self.Sem_gcn(self.bert_lstm_att_feature, attention_mask, seq_lens)

        h1 = self.TIN(h, h_syn_ori, h_syn_gcn, h_sem_ori, h_sem_gcn, adj_sem_ori, adj_sem_gcn)
        h = self.attention_layer(h1, h1, attention_mask[:, :seq_lens]) + h1
